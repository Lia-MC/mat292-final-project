...
class HumanThreatModel:
    ...
    def adams_bashforth_4(self, f, y0, t_span, args, n_steps=1000):
        ...
        # adams methods need history -> use Euler for first 3 points
        for i in range(3):
        ...
        for i in range(3, n_steps):
            f_n   = f(y[i],   t[i],   *args)
            f_n1  = f(y[i-1], t[i-1], *args)
            f_n2  = f(y[i-2], t[i-2], *args)
            f_n3  = f(y[i-3], t[i-3], *args)
...
class AnimalThreatModel:
    ...
    # using ab4 solve for the max threat parameter
    def _find_actual_max_threat(self, a, b, c, d, time_span=200, n_steps=2000):
        y0 = np.array([1.0, 0.1])

        t, y = self.adams_bashforth_4_system(
    ...
    # adams 4th order system
    def adams_bashforth_4_system(self, f, y0, t_span, args, n_steps=1000):
        t0, tf = t_span
        h = (tf - t0) / n_steps
        t = np.linspace(t0, tf, n_steps + 1)
    ...
    def calculate_animal_threat(self, time_span, predator_level=None, initial_survival=1.0, n_steps=1000):
        ...
        # solve ODE system by calling ab4
        t, y = self.adams_bashforth_4_system(
            self.animal_threat_ode,
            y0=y0,
            t_span=(0, time_span),
