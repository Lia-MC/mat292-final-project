...
def bdf2_system1(f, y0, t, tol=1e-8, max_iter=20):
    n = len(t)
    m = len(y0)
    y = np.zeros((n, m))
    y[0] = y0

    # initialize w implicit euler for first step)
    h = t[1] - t[0]
    y[1] = backward_euler_step(f, y[0], t[0], h, tol, max_iter)
    ...
    for i in range(2, n):
        ...
        for _ in range(max_iter):
            G = (3*y_guess - 4*y[i-1] + y[i-2])/(2*h) - f(y_guess, t[i])
            J = numerical_jacobian(lambda z: (3*z - 4*y[i-1] + y[i-2])/(2*h) - f(z, t[i]), y_guess)
        ...
        y_guess[1] = np.clip(y_guess[1], 0.0, 1.0)
        y_guess[0] = max(y_guess[0], 0.0)

        y[i] = y_guess

    return y

def backward_euler_step(f, y_n, t_n, h, tol=1e-8, max_iter=20):
    y = y_n.copy()

    for _ in range(max_iter):
        G = y - y_n - h*f(y, t_n + h)
        J = numerical_jacobian(lambda z: z - y_n - h*f(z, t_n + h), y)
...
def backward_euler_scalar(f, y_n, t_n, h, args, tol=1e-10, max_iter=20):
    y = y_n

    for _ in range(max_iter):
        G = y - y_n - h * f(y, t_n + h, *args)

        eps = 1e-6
        df = (f(y + eps, t_n + h, *args) - f(y, t_n + h, *args)) / eps
...
def backward_euler_system(f, y_n, t_n, h, args, tol, max_iter):
    y = y_n.copy()
    ...
    for _ in range(max_iter):
        ...
        J = numerical_jacobian(
            lambda z: z - y_n - h * f(z, t_n + h, *args),
            y
        )
...
class HumanThreatModel:
    ...
    def bdf2(self, f, y0, t_span, args, n_steps=1000, tol=1e-10, max_iter=20):
        ...
        y[1] = backward_euler_scalar(f, y[0], t[0], h, args, tol, max_iter)
        ...
        for i in range(1, n_steps):
            ...
            for _ in range(max_iter):
                G = (3*y_guess - 4*y[i] + y[i-1])/(2*h) - f(y_guess, tn1, *args)

                eps = 1e-6
                df = (
                    f(y_guess + eps, tn1, *args)
                    - f(y_guess, tn1, *args)
                ) / eps
...
class AnimalThreatModel:
    ...
    def bdf2_system(self, f, y0, t_span, args, n_steps=1000, tol=1e-9, max_iter=20):
        t0, tf = t_span
        h = (tf - t0) / n_steps
        t = np.linspace(t0, tf, n_steps + 1)

        dim = len(y0)
        y = np.zeros((dim, n_steps + 1))
        y[:, 0] = y0

        # backward euler step
        y[:, 1] = backward_euler_system(f, y[:, 0], t[0], h, args, tol, max_iter)

        apply_constraints(y[:, 1])
        ...
        for i in range(1, n_steps):
            ...
            for _ in range(max_iter):
                G = (3*y_guess - 4*y[:, i] + y[:, i-1])/(2*h) \
                    - f(y_guess, tn1, *args)

                J = numerical_jacobian(
                    lambda z: (3*z - 4*y[:, i] + y[:, i-1])/(2*h)
                            - f(z, tn1, *args),
                    y_guess
                )
    ...
    def calculate_animal_threat(self, time_span, predator_level=None, initial_survival=1.0, n_steps=1000):
        ...
        # solve ODE system by calling bdf2
        t, y = self.bdf2_system(
            self.animal_threat_ode,
            y0=y0,
            t_span=(0, time_span),
