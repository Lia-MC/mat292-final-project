'''
Each pixel in the image represents a simulated scenario for your survival model.
The color at each pixel shows the survival probability output from your model, using the "viridis" colormap

AI report explanation:
“This Mandelbrot Survival Fractal visualizes the outputs of my survival model across a multidimensional 
scenario space. Each pixel represents one scenario, with risk, resource availability, and environmental risk 
assigned based on its position in the fractal and escape time algorithm. Survival probability is calculated 
using our model and mapped to color: dark regions indicate poor survival likelihood, while brighter regions 
reflect better survival. The fractal boundary demonstrates the sensitivity of survival probability to small 
changes in model inputs.”

Escape:
Escape speed = Number of iterations before |z| > 2
Represents ‘how quickly things get unstable’ for that scenario
Mapped to risk in your model for visualizing survival probability
'''
