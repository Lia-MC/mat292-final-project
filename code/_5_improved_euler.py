...
def age():
    ...
    sol = improved_euler_system(derivatives, y0, t)
...
# improved euler for a system of ODEs
def improved_euler_system(f, y0, t):
    y = np.zeros((len(t), len(y0)))
    y[0, :] = y0
    ...
    for i in range(1, len(t)):
        ...
        y_pred = y_current + dt * f(y_current, t_current)

        y_next = y_current + 0.5 * dt * (
            f(y_current, t_current) + f(y_pred, t_current + dt)
        )

        y_next[1] = np.clip(y_next[1], 0.0, 1.0) # H stays in [0,1]
        y_next[0] = max(y_next[0], 0.0) # R stays ≥ 0
